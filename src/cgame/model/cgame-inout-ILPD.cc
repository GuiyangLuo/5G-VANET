...
NS_LOG_COMPONENT_DEFINE("cgameinoutlinerpr");
namespace cgame {

ns3::TypeId cgameinoutlinerpr::GetTypeId(void) {
	ns3::TypeId tid =
			ns3::TypeId("cgame::cgameinoutlinerpr").SetParent<ns3::Object>().AddConstructor<
					cgame::cgameinoutlinerpr>().AddAttribute("m_NeighborNumber",
					...
					ns3::MakeUintegerAccessor(
							&cgameinoutlinerpr::m_NeighborNumber),
					ns3::MakeUintegerChecker<uint32_t>());
	return tid;
}
cgameinoutlinerpr::cgameinoutlinerpr() {
	NS_ASSERT(false);
	setCallbackForReceiveFromMaclayer();

}
cgameinoutlinerpr::cgameinoutlinerpr(double comRangeRsu, double comRangeObu,
...
void cgameinoutlinerpr::receiveFromMaclayer(ns3::Ptr<cgame::GraphUpLoad> up) {
	m_receivedTimes++;
	//up->print();
	m_NodeInfor.insert(
...
void cgameinoutlinerpr::printAllTTnew() {
...
void cgameinoutlinerpr::printCOnflictgraph() {
...
void cgameinoutlinerpr::printDirectedNeighborGraph() {
	uint32_t first = 0, second = 0;
...
void cgameinoutlinerpr::printNodeInfor() {
...
void cgameinoutlinerpr::SchedulingBegin() {
	...
	ConvertToLinerProgramingAndSolve();
...
void cgameinoutlinerpr::DeleteSameForAllTTnews() {
...
void cgameinoutlinerpr::HandlingResults() {
...
int cgameinoutlinerpr::BalancedRate(uint32_t sender, std::set<uint8_t> items,
...
void cgameinoutlinerpr::GreedyAlgorithm() {
...
void cgameinoutlinerpr::ConvertToLinerProgramingAndSolve() {
	int numberResults = m_allTTnews.size();

	/*
	 *
	 * The algorithm  for ILP based decomposition algorithm
	 * We will release this code once this paper is accepted!
	 *
	 *
	 */
	...
	for (int i = 0; i < numberResults; i++) {
		if (result[i] == true) {
			m_ChoosedIdsForTTnews.insert((uint32_t) i);
		}
	}
}
void cgameinoutlinerpr::getWeightForEachTTnew() {
...
void cgameinoutlinerpr::CheckForConfilct() {
...
bool cgameinoutlinerpr::isEqualForTwoSet(std::set<uint8_t> set1,
...
bool cgameinoutlinerpr::isEqualForTwoSet32(std::set<uint32_t> set1,
...
void cgameinoutlinerpr::GetAllTTnews() {
...
void cgameinoutlinerpr::GetAllTTnewsReduced() {
	std::map<uint32_t, ns3::Ptr<cgame::GraphUpLoad> >::iterator sender;
...
void cgameinoutlinerpr::GetTTnewsForCertainReceivers(uint32_t sender,
...
void cgameinoutlinerpr::alpha2TimesReducedForItems(uint32_t i,
...
void cgameinoutlinerpr::alpha2TimesReducedForReceivers(uint32_t i,
...
std::set<uint8_t> cgameinoutlinerpr::OrforTwoSet(std::set<uint8_t> alpha,
...
void cgameinoutlinerpr::GetTTnewsForCertainNode(uint32_t senderId,
...
void cgameinoutlinerpr::alpha2Times(uint32_t i, std::vector<uint8_t> Temp) {
	std::vector<uint8_t> reTemp;
	reTemp = Temp;
	if (i == m_alpha.size()) {
...
std::set<uint8_t> cgameinoutlinerpr::ConvertVectorToSet(
...
void cgameinoutlinerpr::SelectOnExponentTTnews(uint32_t senderId,
		std::set<uint8_t> alpha, std::vector<uint32_t> allneighbors,
		std::set<uint32_t> allneigborsOfChannel) {
...
int cgameinoutlinerpr::PrintNumberOfOriginsTTnew(uint32_t senderId,
		std::set<uint8_t> alpha, std::vector<uint32_t> allneighbors,
		std::set<uint32_t> allneigborsOfChannel) {
	int totalNumber = 0;
...
void cgameinoutlinerpr::GetAllPossibleTransmissionSet(uint32_t senderId,
		std::set<uint8_t> alpha, std::vector<uint32_t> allneighbors,
		std::set<uint32_t> allneigborsOfChannel) {
	NS_ASSERT(1 == 0);
	m_alpha.clear();
...
std::set<uint32_t> cgameinoutlinerpr::VectorToSet(uint32_t senderId,
...
void cgameinoutlinerpr::GetChannelInfor() {
	std::map<uint32_t, ns3::Ptr<cgame::GraphUpLoad> >::iterator sender,
			receiver;
...
///
std::set<uint8_t> cgameinoutlinerpr::AndforTwoSet(std::set<uint8_t> alpha,
		std::set<uint8_t> beta) {
	std::set<uint8_t> results;
	results.clear();
	std::set<uint8_t>::iterator it1;
...
void cgameinoutlinerpr::setCallbackForReceiveFromMaclayer() {
	...
	for (ns3::NodeList::Iterator i = ns3::NodeList::Begin();
		...
		(*i)->GetDevice(0)->GetObject<cgamemac::CgamemacNetDevice>()->SetGraphUpLoadCallback(
				ns3::MakeCallback(&cgameinoutlinerpr::receiveFromMaclayer,
						this));

	}

}
void cgameinoutlinerpr::SendToMaclayer(ns3::Ptr<cgame::cgameDownload> down) {
...
void cgameinoutlinerpr::sendAdToMaclayer(
...
void cgameinoutlinerpr::ClearForPerScheduling() {
