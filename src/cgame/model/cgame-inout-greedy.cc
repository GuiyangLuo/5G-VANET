...
NS_LOG_COMPONENT_DEFINE("cgameinoutgreedy");
namespace cgame {

ns3::TypeId cgameinoutgreedy::GetTypeId(void) {
	ns3::TypeId tid =
			ns3::TypeId("cgame::cgameinoutgreedy").SetParent<ns3::Object>().AddConstructor<
					cgame::cgameinoutgreedy>().AddAttribute("m_NeighborNumber",
					...
					ns3::MakeUintegerAccessor(
							&cgameinoutgreedy::m_NeighborNumber),
					ns3::MakeUintegerChecker<uint32_t>());
	return tid;
}
cgameinoutgreedy::cgameinoutgreedy() {
	NS_ASSERT(false);
	setCallbackForReceiveFromMaclayer();

}
cgameinoutgreedy::cgameinoutgreedy(double comRangeRsu, double comRangeObu,
...
void cgameinoutgreedy::receiveFromMaclayer(ns3::Ptr<cgame::GraphUpLoad> up) {
	m_receivedTimes++;
	//up->print();
	m_NodeInfor.insert(
...
void cgameinoutgreedy::printAllTT() {
...
void cgameinoutgreedy::printCOnflictgraph() {
...
void cgameinoutgreedy::printDirectedNeighborGraph() {
	uint32_t first = 0, second = 0;
...
void cgameinoutgreedy::printNodeInfor() {
...
void cgameinoutgreedy::SchedulingBegin() {
...
void cgameinoutgreedy::DeleteSameForAllTTs() {
...
void cgameinoutgreedy::HandlingResults() {
...
void cgameinoutgreedy::GreedyAlgorithm() {
...
void cgameinoutgreedy::getWeightForEachTT() {
...
void cgameinoutgreedy::CheckForConfilct() {
...
bool cgameinoutgreedy::isEqualForTwoSet(std::set<uint8_t> set1,
...
void cgameinoutgreedy::GetAllTTs() {
...
void cgameinoutgreedy::GetAllTTsReduced() {
	std::map<uint32_t, ns3::Ptr<cgame::GraphUpLoad> >::iterator sender;
...
void cgameinoutgreedy::GetTTsForCertainReceivers(uint32_t sender,
...
void cgameinoutgreedy::alpha2TimesReducedForItems(uint32_t i,
...
void cgameinoutgreedy::alpha2TimesReducedForReceivers(uint32_t i,
...
std::set<uint8_t> cgameinoutgreedy::OrforTwoSet(std::set<uint8_t> alpha,
...
void cgameinoutgreedy::GetTTsForCertainNode(uint32_t senderId,
...
void cgameinoutgreedy::alpha2Times(uint32_t i, std::vector<uint8_t> Temp) {
	std::vector<uint8_t> reTemp;
	reTemp = Temp;
	if (i == m_alpha.size()) {
...
std::set<uint8_t> cgameinoutgreedy::ConvertVectorToSet(
...
void cgameinoutgreedy::SelectOnExponentTTs(uint32_t senderId,
		std::set<uint8_t> alpha, std::vector<uint32_t> allneighbors,
		std::set<uint32_t> allneigborsOfChannel) {
...
int cgameinoutgreedy::PrintNumberOfOriginsTT(uint32_t senderId,
		std::set<uint8_t> alpha, std::vector<uint32_t> allneighbors,
		std::set<uint32_t> allneigborsOfChannel) {
	int totalNumber = 0;
...
void cgameinoutgreedy::GetAllPossibleTransmissionSet(uint32_t senderId,
		std::set<uint8_t> alpha, std::vector<uint32_t> allneighbors,
		std::set<uint32_t> allneigborsOfChannel) {
	m_alpha.clear();
...
std::set<uint32_t> cgameinoutgreedy::VectorToSet(uint32_t senderId,
...
void cgameinoutgreedy::GetChannelInfor() {
	std::map<uint32_t, ns3::Ptr<cgame::GraphUpLoad> >::iterator sender,
			receiver;
...
///
std::set<uint8_t> cgameinoutgreedy::AndforTwoSet(std::set<uint8_t> alpha,
		std::set<uint8_t> beta) {
	std::set<uint8_t> results;
	results.clear();
	std::set<uint8_t>::iterator it1;
...
void cgameinoutgreedy::setCallbackForReceiveFromMaclayer() {
	...
	for (ns3::NodeList::Iterator i = ns3::NodeList::Begin();
		...
		(*i)->GetDevice(0)->GetObject<cgamemac::CgamemacNetDevice>()->SetGraphUpLoadCallback(
				ns3::MakeCallback(&cgameinoutgreedy::receiveFromMaclayer,
						this));

	}

}
void cgameinoutgreedy::SendToMaclayer(ns3::Ptr<cgame::cgameDownload> down) {
...
void cgameinoutgreedy::sendAdToMaclayer(
...
void cgameinoutgreedy::ClearForPerScheduling() {
