...
private:
	...
	void ReceiveSelf(uint32_t i, Ptr<Packet> packet, double rxPowerDbm,
			WifiTxVector txVector, Temp temp) const;
	...
	//self defined
	uint32_t m_probOfObstacle;
	uint32_t m_distancecommunication;
	ns3::UniformVariable m_uniRanVar;
	/*Obstacles obs;*/

	uint32_t m_nearRange;
};
